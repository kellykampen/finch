...
describe("runRepost", () => {
  test("reposts a bare id", async () => {
    ...
    const result = await runRepost(["999"], { getTransport: () => transport });
  ...
  test("extracts the id from a status URL", async () => {
    ...
    const result = await runRepost(["https://x.com/user/status/999"], { getTransport: () => transport });
  ...
  test("throws USAGE_ERROR when the id-or-url argument is missing", async () => {
    await expect(runRepost([], { getTransport: () => fakeTransport({}) })).rejects.toThrow(FinchError);
  });
