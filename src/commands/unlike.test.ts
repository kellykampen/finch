...
describe("runUnlike", () => {
  test("unlikes a bare id", async () => {
    ...
    const result = await runUnlike(["999"], { getTransport: () => transport });
  ...
  test("extracts the id from a status URL", async () => {
    ...
    const result = await runUnlike(["https://x.com/user/status/999"], { getTransport: () => transport });
  ...
  test("throws USAGE_ERROR when the id-or-url argument is missing", async () => {
    await expect(runUnlike([], { getTransport: () => fakeTransport({}) })).rejects.toThrow(FinchError);
  });
