...
// (`--json`, `--help`/`-h`, `--describe`, `--version`/`-v`) are only ever
// recognized or stripped from the global-flags region: a literal positional
// value that happens to equal one of those strings (e.g. post text of "--json")
// must never be misinterpreted as the flag, and must never be silently deleted.
